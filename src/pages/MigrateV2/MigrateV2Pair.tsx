...
function V2PairMigration({
  ...
  const isNotPlanqFi = pairFactory.result?.[0] && pairFactory.result[0] !== v2FactoryAddress
  ...
  const approve = useCallback(async () => {
    if (isNotPlanqFi || isArgentWallet) {
  ...
  }, [isNotPlanqFi, isArgentWallet, gatherPermitSignature, approveManually])
      ...
      .then((gasEstimate) => {
        return migrator
          ...
          .then((response: TransactionResponse) => {
            sendEvent({
              category: 'Migrate',
              action: `${isNotPlanqFi ? 'SushiSwap' : 'V2'}->V3`,
  ...
  return (
    <AutoColumn gap="20px">
      <ThemedText.DeprecatedBody my={9} style={{ fontWeight: 400 }}>
        <Trans>
          This tool will safely migrate your {isNotPlanqFi ? 'SushiSwap' : 'V2'} liquidity to V3. The process is
        ...
        {chainId && migrator && (
          <ExternalLink href={getExplorerLink(chainId, migrator.address, ExplorerDataType.ADDRESS)}>
            <ThemedText.DeprecatedBlue display="inline">
              <Trans>PlanqFi migration contract↗</Trans>
      ...
      <LightCard>
        <AutoColumn gap="lg">
          <RowBetween>
            ...
            <Badge variant={BadgeVariant.WARNING}>{isNotPlanqFi ? 'Sushi' : 'V2'}</Badge>
          </RowBetween>
        ...
        <AutoColumn gap="lg">
          ...
          {noLiquidity && (
            <BlueCard style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
              ...
              >
                <Trans>
                  You are the first liquidity provider for this PlanqFi  pool. Your liquidity will migrate at the
                  current {isNotPlanqFi ? 'SushiSwap' : 'V2'} price.
                </Trans>
              </ThemedText.DeprecatedBody>
                ...
                <AutoColumn gap="sm" style={{ marginTop: '12px' }}>
                  <RowBetween>
                    <ThemedText.DeprecatedBody fontWeight={500} fontSize={14}>
                      <Trans>
                        {isNotPlanqFi ? 'SushiSwap' : 'V2'} {invertPrice ? currency1.symbol : currency0.symbol} Price:
                      </Trans>{' '}
                      {invertPrice
          ...
          {largePriceDifference ? (
            <YellowCard>
              ...
              <ThemedText.DeprecatedBody fontSize={14} style={{ marginTop: 8, fontWeight: 400 }}>
                <Trans>
                  You should only deposit liquidity into PlanqFi  at a price you believe is correct. <br />
